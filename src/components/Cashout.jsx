...
const Cashout = ({token, user }) => {
    const [receiver, setReceiver] = useState('');
    const [amount, setAmount] = useState('');
    const navigate = useNavigate();
    const remainingBalance = user.balance - (parseFloat(amount) + parseFloat(amount) * 0.02);
        ...
        <div>
            <div className='flex justify-between px-[300px] my-[80px] w-[full]'>
                <div>
                    ...
                    <p className='font-[400] text-[20px] text-gray-400'>Total Cost: {amount && (parseFloat(amount) + parseFloat(amount) * 0.02)}</p>
                    {amount > 0 && (
